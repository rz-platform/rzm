...
import users.controllers.{ AccountController, AuthController, AuthenticatedAction, routes => usersRoutes }
import users.models.{ Account, AccountInfo, UserRegistration }
...
class GenericControllerTest
  ...
  def collaboratorsRepository: CollaboratorRepository = app.injector.instanceOf[CollaboratorRepository]
  ...
  def createAccount(): AuthorizedAccount = {
    ...
    val account: Either[RepositoryError, Account] = await(accountRepository.getByName(data.username))
    account match {
      case Right(account) =>
  ...
  def createRepository(
    ...
    val request = addCSRFToken(
      FakeRequest(documentsRoutes.RzRepositoryController.save())
        .withFormUrlEncodedBody("name" -> name, "description" -> getRandomString)
        .withSession(owner.s)
        .withCookies(owner.c)
    )
    val result                                      = await(gitEntitiesController.save().apply(request))
    val repo: Either[RepositoryError, RzRepository] = await(rzGitRepository.getByOwnerAndName(owner.a.username, name))
    repo match {
